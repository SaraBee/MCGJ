...
- Create the file `.env` in the top directory of the repo, starting from a copy
  ...
  cp template.env .env
  ```
  In `.env`, set `ROOT_URL` to the external URL of this deployment.  This is
  required for all OAuth features, including Spotify integration and OAuth
  login via recurse.com.
